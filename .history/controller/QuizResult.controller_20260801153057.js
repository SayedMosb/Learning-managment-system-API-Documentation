...
const getMyResults =async(req,res)=>{
    try{
        const users = await QuizResult.find({student: req.currentUser.id}).populate("Quiz");;
        if(users.length===0){
        return res.status(400).json({msg:'no Quiz is found'});
        }
      return res.status(200).json({msg: "Quiz results retrieved successfully",results: users});
    }catch(err){
...
const getallresult = async(req,res)=>{
    try{
      ...
      if (quizesResults.length === 0) {
    return res.status(404).json({msg: "No quiz results found"});
}
       return res.status(200).json({data:quizesResults});
    }catch(err){
       return res.status(500).json({msg: err.message});
    }
}
