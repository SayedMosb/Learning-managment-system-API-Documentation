...
const updatedCategory = async(req,res)=>{
    try{
      const updated =  await categorys.findByIdAndUpdated(
        req.params.id,
        req.body,
        {new:true}
      );
      if(!updated){
        res.status(500).json({message:'all filed are null'});
      }
      res.status(201).json({data:updated});
    }catch(err){
     res.status(404).json({message:'error to updated in data'});
    }
}

const deletedCategory = async(req,res)=>{
    try{
      const delted = await categorys
    }catch(err){
        res.status(500).json({message:'error when you deledted '});
    }
}
