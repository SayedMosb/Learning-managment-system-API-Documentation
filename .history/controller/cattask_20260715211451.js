...
const addcategory =async(req,res)=>{
    try{
     const {name}=req.body;
     if(!name){
        return res.status(202).json({message:'name of category are required'});
     }

     const newdata = await categorys.create({
        name
     });
     return res.status(202).json({message:newdata});
    }catch(err){
return res.status(404).json({message:'error to category adding'});
    }
}
...
const deletedCategory = async(req,res)=>{
    try{
      const deleted = await categorys.findByIdAndDelete(req.params.id);
      if (!deleted) {
    return res.status(404).json({
        message: "Category not found"
    });
...
module.exports ={addcategory,getallCategory,getallCategoryById,updatedCategory,deletedCategory};