...
const updatedWishlist = async(req,res)=>{
    try{
        ...
        ,{new:true}
          ...
          if(!updated){
        return   res.status(400).json({msg:'wishlist is not found to updated'});
          }
...
const deletedWishlist = async(req,res)=>{
    try{
            const deleted = await Wishlist.findByIdAndDelete(req.params.id);
            if(!deleted){
                return  res.status(400).json({mes:'wishlish is not found to deleted'});
            }
            return  res.status(200).json({mes:'wishlist is deleted successful'});
    }catch(err){
       return  res.status(500).json({mes:err.message});
    }
}