...
const updatedWishlist = async(req,res)=>{
    try{
          const updated = await Wishlist.findByIdAndUpdate({student:req.currentUser._id,Course:req.currentUser._id});
          if(!updated){
            res.status(400).json({msg:'wishlist is not found to updated'});
          }
          res.status(200)
    }catch(err){
